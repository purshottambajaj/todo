...
const YettoStart = () => {
    ...
    const fetchTasks = async () => {
        try {
            const response = await fetch('http://localhost:5000/getapi/tasks?status=yet-to-start');
            if (response.ok) {
    ...
    const handleNext = async (taskId, taskName) => {
        console.log(taskName);

        try {
            const response = await fetch(`http://localhost:5000/update/tasks/${taskName}/in-progress`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div className='flex flex-col'>
            <div className='flex flex-col items-center px-10 py-30'>
                <span className=''>Yet to Start</span>
